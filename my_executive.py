...
class MyExecutor(Executor):
    ...
    def initialize(self, services):
        ...
        self.is_policy_exists = path.exists(self.policy_file) and path.getsize(self.policy_file) > 0
    ...
    def next_action(self):
        ...
        if self.converted_to_deterministic and not self.hidden_objects:
            action = self.new_deterministic_next_action(full_current_state, valid_actions)
            return action
    ...
    def check_if_deterministic(self):
        try:
            self.planner_steps = self.get_plan(self.domain_file_name, self.problem_file_name)
    ...
    def get_non_recently_random_action(self, valid_actions):
        ...
        for act in self.last_actions:
            try:
                ...
                pass
        if self.previous_state is not None and self.current_state == self.previous_state:
    ...
    def new_deterministic_next_action(self, full_current_state, valid_actions):
        ...
        new_problem_content = self.init_problem_content.replace(self.init_predicates_str, current_predicates_str)
        ...
        non_deterministic_action = deterministic_action.replace(det_action_name, non_det_action_name)
...
if input_flag == '-L':  # Learning phase
    print LocalSimulator().run(domain_path, problem_path, QLearningExecutor(domain_path, problem_path))
elif input_flag == '-E':  # Execution phase
    print LocalSimulator().run(domain_path, problem_path, MyExecutor(domain_path, problem_path))
else:
